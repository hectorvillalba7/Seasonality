...
def run():
    st.title("Seasonality of USD futures")

    ticker = "DX-Y.NYB"

    # date selection
    start = st.date_input("Starting Date", value=dt.datetime(1978, 1, 1))
  ...
  #---------------------Bar chart divided per month.---------------------
    ...
    selected_chart = st.selectbox("Select which month do you want to display:", options)
  ...
  #Data source to create the charts
    st.subheader("USD Data from Yahoo Finance")
